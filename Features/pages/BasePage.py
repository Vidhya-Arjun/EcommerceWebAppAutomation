...
class BasePage:
    ...
    def open_login_page(self):
        url = ConfigReader.read_configuration("basic info", "base_url")
        self.driver.get(url)
    ...
    def get_elements(self, locator_name, locator_value):
        ...
        if locator_name.endswith("_id"):
            ...
            elements = self.wait.until(
                EC.presence_of_all_elements_located((By.NAME, locator_value))
            )

        elif locator_name.endswith("_xpath"):
            elements = self.wait.until(
                EC.presence_of_all_elements_located((By.XPATH, locator_value))
            )

        elif locator_name.endswith("_css"):
            elements = self.wait.until(
                EC.presence_of_all_elements_located((By.CSS_SELECTOR, locator_value))
            )

        elif locator_name.endswith("_class"):
            elements = self.wait.until(
                EC.presence_of_all_elements_located((By.CLASS_NAME, locator_value))
            )

        elif locator_name.endswith("_linktext"):
            elements = self.wait.until(
                EC.presence_of_all_elements_located((By.LINK_TEXT, locator_value))
            )
